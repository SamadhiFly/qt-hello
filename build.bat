...
if exist "%CMAKE_PRESETS_FILE%" (
    echo ### configuring...
    cmake --preset %CMAKE_PRESETS_CONFIG%
)

if exist "%BUILD_NINJA_FILE%" (
    echo ### building...
    cmake --build --preset %CMAKE_PRESETS_BUILD% --target all
)

if exist "%TARGET_FILE_NAME%" (
    echo ### removing target %TARGET_FILE_NAME%
    move /y "%TARGET_FILE_NAME%" "%TARGET_FILE_NAME%%TARGET_FILE_EXT%" 2>nul
)
